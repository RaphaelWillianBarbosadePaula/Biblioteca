function saveUser() {
    ...
    if (userId) {
      ...
      fetch("../models/editoras/editarEditoras.php", {
        method: "POST",
      ...
      fetch("../models/editoras/cadastrarEditoras.php", {
        method: "POST",
        body: JSON.stringify(data),
        headers: { "Content-type": "application/json; charset=UTF-8" },
      })
  ...
  function consultar(userId) {
    fetch(`../models/editoras/consultarEditoras.php?userId=${userId}`)
      .then(function (response) {
        response.json().then(function (data) {
          var modalElement = document.getElementById("modalEditoras");
          var modalEditoras = bootstrap.Modal.getInstance(modalElement);
          ...
          document.getElementById("modalTitle").textContent = "Consultar Editoras";
        });
      })
      .catch(function (err) {
        console.error("Erro", err);
    ...
    fetch(`../models/editoras/consultarEditoras.php?userId=${userId}`)
      .then(function (response) {
        response.json().then(function (data) {
          ...
          document.getElementById("modalTitle").textContent = "Editar Editoras";
        });
      })
      .catch(function (err) {
        console.error("Erro", err);
  ...
  function excluir(userId) {
    var modalElementDelete = document.getElementById("modalExcluirEditoras");
    var modalExcluirEditoras = bootstrap.Modal.getInstance(modalElementDelete);
    modalExcluirEditoras.show();
  ...
  function realDeleteUser() {
    ...
    fetch(`../models/editoras/excluirEditoras.php?userId=${deleteUserId}`, {
      method: "DELETE",
    })
      .then(function (response) {
        $("#modalExcluirEditoras").modal("hide");
